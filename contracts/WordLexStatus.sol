...
contract WordLexStatus is Ownable {
    ...
    function buyStatus(uint40 _id, address payable _upLiner) public payable {
        ...
        require(
            users[msg.sender].status == 0,
            "WDXStatus:Status already bought"
        );
        require(
            _upLiner != address(0) && users[_upLiner].status > 0,
            "WDXStatus: UpLiner doesn't exist"
        );
        ...
        if (_upLiner != address(0)) {
            users[_upLiner].directBonus = users[_upLiner].directBonus.add(
    ...
    function upgradeStatus(uint40 _id) public payable {
        ...
        require(
            msg.value ==
                getStatusPrice(_id).sub(
                    getStatusPrice(users[msg.sender].status)
                ),
            "WDXStatus:Bad Amount"
        );
        users[msg.sender].status = _id;
    }
    ...
    function addStatus(
        ...
        statuses.push(
            Status({
                usdPrice: _usdPrice,
                weeklyLimitUSD: _weeklyLimitUSD,
                lines: _lines,
                name: _name
            })
        );
    ...
    function getAddressStatus(address _statusHolder)
        public
        view
        returns (uint256)
    {
    ...
    function getStatusUSDPrice(uint256 _statusId)
        external
        view
        returns (uint256)
    {
    ...
    function getStatusLimit(uint256 _statusId) external view returns (uint256) {
        return
            statuses[_statusId].weeklyLimitUSD.mul(
                controller.getCurrentUsdRate()
            );
    }
    ...
    function getStatusName(uint256 _statusId)
        external
        view
    ...
    function _refPayout(address _addr, uint256 _amount) internal {
        ...
        require(
            refBonuses.length <= getAddressStatus(_addr),
            "WDXStatus:Unavailable lines"
        );
