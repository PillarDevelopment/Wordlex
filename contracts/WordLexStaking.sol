...
contract WordLexStaking is Ownable {
    ...
    function withdraw(uint256 _amount) public {
        require(
            ...
            "WordLexStaking:incorrect amount, try less"
        );
        require(
            users[msg.sender].withdrawTime.add(7 days) < block.timestamp,
    ...
    function setRefBonusesPercentage(uint256 line, uint8 newAmount)
        public
        onlyOwner
    {
    ...
    function maxDailyPayoutOf(address _statusHolder)
        public
        view
        returns (uint256)
    {
        return
            statusContract.getStatusLimit(
                statusContract.getAddressStatus(_statusHolder)
            );
    }

    function payoutOf(address _addr, uint256 _amount)
        public
        view
    ...
    function getDailyPercent(address _addr)
        public
        view
        returns (uint256 _dailyPercent)
    {
        return
            minimumDailyPercent
                .add(getDepositHoldBonus(users[_addr].depositAmount))
                .add(getTimeBonus(users[_addr].depositTime));
    }

    function getTimeBonus(uint256 _depositTime)
        public
        view
        returns (uint256 _dailyTimeBonus)
    {
    ...
    function getDepositHoldBonus(uint256 _depositAmount)
        public
        pure
        returns (uint256 _dailyHoldBonus)
    {
        if (_depositAmount > 10000000000) {
            // 10,000 WDX
            _dailyHoldBonus = 1;
        }
        if (_depositAmount > 20000000000) {
            // 20,000 WDX
            _dailyHoldBonus = 2;
        }
        if (_depositAmount > 100000000000) {
            // 100,000 WDX
            _dailyHoldBonus = 3;
        }
        if (_depositAmount > 200000000000) {
            // 200,000 WDX
            _dailyHoldBonus = 4;
        }
    ...
    function generateCompoundInterest(address _addr, uint256 amount)
        public
        view
        returns (uint256)
    {
        ...
        if (accDay > 1) {
            for (uint256 i = 0; i < accDay; i++) {
                accAmount = accAmount.add(
                    accAmount.mul(currentPercent).div(1000)
                );
            }
        } else {
            accAmount = 0;
