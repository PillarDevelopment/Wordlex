...
contract AutoProgramWordLex is Ownable {
    ...
    function registration(address _upline, uint256 _carPrice, address _addr) public onlyOwner {
        require(!liquidateUsers[msg.sender], "AutoProgramWDX: This address liquidated");
    ...
    function withdraw() public {
        require(users[msg.sender].activeAccount ||
            (users[msg.sender].investTime.add(180 days) < block.timestamp && getsBuyersIn1Line(msg.sender) > 1),
        ...
        uint256 currentWithdraw = match_bonus.add(users[msg.sender].direct_bonus);
    ...
    function liquidateInactiveAccount(address inactiveAccount) external {
        require(liquidateUsers[msg.sender] == false, "AutoProgramWDX: This address liquidated");
        require(users[msg.sender].statusOfCar == true, "AutoProgramWDX: Sender is not Active Account");
        require(users[inactiveAccount].statusOfCar == false || users[inactiveAccount].totalStructure == 0,
            "AutoProgramWDX: This address bought a car");

        require(users[inactiveAccount].investTime.add(180 days) < now, "AutoProgramWDX: Need more time");
        require(users[inactiveAccount].carPrice != 0,
                "AutoProgramWDX: This address isn't inactive or not available");

        _refPayout(inactiveAccount, users[inactiveAccount].carPrice);
        uint256 liquidationAmount = users[inactiveAccount].match_bonus.add(users[inactiveAccount].direct_bonus);
    ...
    function getAddressFirstLine(address _addr) public view returns(uint256[] memory) {
